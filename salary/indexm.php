...
if (isset($_POST['pass']) && isset($_POST['email'])) {
  if ( strlen($_POST['email']) < 1 || strlen($_POST['pass']) < 1 ) {
    ...
    $stmt = $pdo->prepare('SELECT Employ_ID,email, Password,MainRole FROM addemploy WHERE email = :em AND Password = :pw AND MainRole = :m');
    $stmt->execute(array(':em' => $_POST['email'], ':pw' =>hash('md5', $salt . $_POST['pass']) , ':m' => $m));
